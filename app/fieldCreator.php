...
$profileListUrl = "https://" . $server . "/exzact/api/v60/profiles?limit=1&access_token=" . $token;
$profileRequestHeaders = (get_headers($profileListUrl,1));
$finalProfileCount = $profileRequestHeaders["Total-Count"];
...
for ($i=0; $i<$finalProfileCount; $i++){
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "https://" . $server . "/exzact/api/v60/profiles?limit=1&offset=$i");
  ...
  curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    ...
    $pageListUrl = "https://" . $server . "/exzact/api/v60/profiles/$activeProfile/pages?limit=1&access_token=" . $token;
    $pageRequestHeaders = (get_headers($pageListUrl,1));
    $finalPageCount = $pageRequestHeaders["Total-Count"];
    ...
    for ($j=0; $j<$finalPageCount; $j++){
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, "https://" . $server . "/exzact/api/v60/profiles/$activeProfile/pages?limit=1&offset=$j");
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
      curl_setopt($ch, CURLOPT_HEADER, FALSE);
      ...
      curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://" . $server . "/exzact/api/v60/profiles/$activeProfile/pages/$activePage/elements");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($ch, CURLOPT_HEADER, FALSE);
